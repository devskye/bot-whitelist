...
interface EmbedOptions extends EmbedPlusData {
  fields?: {
    name: string;
    value: string;
    inline?: boolean;
...
type ResFunction = (text: string, options?: EmbedOptions) => any;
type Res = Record<keyof SettingsColors, ResFunction>;
...
export const res: Res = Object.create({}, Object.entries(settings.colors)
  .reduce((obj, [name, color]) => Object.assign(obj, {
    [name]: {
      enumerable: true,
      writable: false,
      value(description: string, options?: EmbedOptions){
        // junta cor, descrição e as opções passadas (incluindo fields)
        const data = Object.assign({ color, description }, options);
        const embed = createEmbed(data);
