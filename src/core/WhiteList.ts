...
export class WhiteListManager {
    ...
    private async initializeData(): Promise<void> {
        try {
            const { questions, whitelists } = await this.whitelistService.initializeWhitelistData();
    ...
    async deleteAllChannels(interaction: any): Promise<void> {
        const deletePromises = Array.from(this.whitelists.values()).map(async (whitelist) => {
            const channel = interaction.guild.channels.cache.get(whitelist.data.channelId);
    ...
    private async defaultMessage(interaction: any, receivedchannel: any) {
        const embed = DiscordService.createWelcomeEmbed(interaction.member.user.displayAvatarURL());
        ...
        const message = await receivedchannel.send({ embeds: [embed], components: [row] });
        ...
        collector.on('end', async (collected: any) => {
            ...
            if (collected.size === 0) {
                if (receivedchannel.deletable) await receivedchannel.delete().catch(() => {});
                await whitelist?.Delete();
                this.whitelists.delete(interaction.user.id);
            }
        });
    }
    public async createWhitelistChannel(interaction: any): Promise<GuildTextBasedChannel | undefined> {
        try {
            const existingWhitelist = this.whitelists.get(interaction.user.id);
            if (existingWhitelist?.data.messageId) {
                await interaction.reply({ content: 'Você já tem um canal de whitelist', ephemeral: true });
                return undefined; // retorna undefined pois não criou canal
            }
    
            const channel: GuildTextBasedChannel = await interaction.guild.channels.create({
    ...
    async editQuestion(id: number, questionText: string): Promise<Question> {
        try {
           const result = await this.whitelistService.updateQuestion(id, questionText);
        ...
        async getWhitelistByUserId(userId: string): Promise<UserWhitelist | null> {
            ...
            try {
                const dbWhitelist = await this.whitelistService.getWhitelistByUserId(userId);
    ...
    async endWhitelist(interaction: any): Promise<void> {
        ...
        try {
            ...
            // Process whitelist finalization after delay
            setTimeout(async () => {
    ...
    private async processWhitelistFinalization(interaction: any, whitelist: UserWhitelist, userId: string): Promise<void> {
        try {
            const { answers, questions } = await this.whitelistService.finalizeWhitelist(userId);
            ...
            const attachment = DiscordService.createFileAttachment(file, `${userId}.txt`);
    ...
    async startWhitelist(interaction: any, receivedchannel: TextBasedChannel): Promise<void> {
        console.log('[START] Chamando startWhitelist para', interaction.user.id);
        try {
            if (this.questions.length === 0) {
                interaction.reply({ content: 'Não há perguntas cadastradas', ephemeral: true });
                return;
            }

            const whitelist = this.whitelists.get(interaction.user.id);
            if (!whitelist) {
                interaction.reply({ content: 'Você não tem um canal de whitelist', ephemeral: true });
                return;
            }
            console.log("whitelist started", whitelist.data.started);

            if (whitelist?.data.started) {
                if (!interaction.replied && !interaction.deferred)
                    await interaction.reply({ content: 'Whitelist já iniciada.', ephemeral: true });
                return;
            }
            this.whitelists.get(interaction.user.id)!.data.started = true;

           await interaction.reply({ content: 'Whitelist iniciada!', ephemeral: true });
    ...
    private async sendQuestionAndCollectAnswer(interaction: any, question: Question, isFirst = false): Promise<void> {
        console.log('[SEND] Enviando pergunta:', question.question);
        const embed = DiscordService.createWhitelistEmbed('Whitelist', question.question);
    ...
    private async handleWhitelistTimeout(interaction: any, whitelist: UserWhitelist): Promise<void> {
    ...
    public async acceptManualWhitelist(interaction: any, user: any): Promise<void> {
        try {
            ...
            interaction.reply({ content: 'Erro ao processar aprovação', ephemeral: true });
        }
    }
    ...
    public async denyManualWhitelist(interaction: any, user: any): Promise<void> {
        try {
            ...
            interaction.reply({ content: 'Erro ao processar negação', ephemeral: true });
        }
    }

    private findWhitelistByMessageId(messageId: string): UserWhitelist | undefined {
    ...
    public async acceptWhitelist(interaction: any): Promise<void> {
        ...
        if (!whitelist) {
            interaction.reply({ content: 'Whitelist não encontrada', ephemeral: true });
            return;
        }
        ...
        try {
            ...
            const logEmbed = DiscordService.createLogEmbed(user.user.username, interaction.user.username, true);
            await DiscordService.sendToChannel(interaction.guild, Settings.whitelist['LOGS-CHANNEL'], { embeds: [logEmbed] });

            const approvalMsg = DiscordService.getApprovalMessage(whitelist.data.userId);
            await DiscordService.sendToChannel(interaction.guild, Settings.whitelist['STATE-CHANNEL'], { content: approvalMsg });
    ...
    public async denyWhitelist(interaction: any): Promise<void> {
        ...
        try {
            ...
            await this.whitelistService.deleteWhitelistByUserId(whitelist.data.userId); 
        }catch (err) {
